...
#                    subtopic, pattern, str(message)))
        ...
        if tail == 'STATE':  # POWER* status
            ...
            updateResultDevice(fullName, cmnd, message)
...
def getSensorDeviceState(states, sensName, attr, value, linkQuality, batteryPercentage):
    typeDb = {
...
def getComposeAttr(attrList):
    ...
    return composeAttr,composeValue
...
#        linkQuality = Value / 10
    ...
    if Attr == None: # not compose attribute
        for Attr, Value in attrList.items():
            getSensorDeviceState(states, sensorName, Attr, Value, None, 255)
    else:
        getSensorDeviceState(states, sensorName, Attr, Value, None, 255)
...
def getZigbeeDeviceState(states, zbDevice, zbName, attr, value, linkQuality, batteryPercentage):
    ...
    'Humidity':           {'Name': 'Humidity',      'Unit': '%',    'DomoType': 'Humidity'},
    'Temp+Hum':           {'Name': 'Temp+Hum',      'Unit': '',     'DomoType': 'Temp+Hum'}, # combine Temp+Hum device if Temperature and Humidity exist in attr list
    'Water':              {'Name': 'Alert',         'Unit': '',     'DomoType': 'Alert'},    # Water sensor
...
def getZigbeeDeviceStateEx(states, attrList):
    ...
    zbDevice = attrList['Device']
    if 'Name' in attrList:
        zbName = attrList['Name']
    else:
        zbName = zbDevice
...
#                    getSensorDeviceStateEx(states,sensor,value)
                    ...
                    if sensor.startswith('Range'): # Range1,Range2 ...
                        ...
                        getSensorDeviceState(states,sensor,'Range',value,None,None)
                    elif(sensor.startswith('A')): #Analog A1,A2 ...
                        getSensorDeviceState(states,sensor,'Range',value,None,None)
                    elif(sensor.startswith('Temperature')): 
                        getSensorDeviceState(states,sensor,'Temperature',value,None,None)
                    elif(sensor.startswith('Light')):
                        getSensorDeviceState(states,sensor,'Illuminance',value,None,None)
...
# Create a domoticz device from infos extracted out of tasmota SENSOR tele messages or RESULT message
def createDevice(mqttName, deviceID, mqttTopic, device, deviceAttr, desc):
...
#    Description contains necessary info as json to send  DomoticzCommand
    ...
    if device.startswith('0x'): #hack: zigbee device type
        attr = attrs[-1]
        if attr.upper() == 'POWER':
    ...
    Domoticz.Log("tasmota::createDevice: deviceID: {}, domoDeviceName: {}, mqttName: {}".format(
...
#        # Remove hardware/plugin name from domoticz device name
        ...
        Domoticz.Log("tasmota::createDevice: mqttName: {}, deviceID: {}, domoDeviceName: {}, domoType: {}".format(
...
def updateResultDevice(mqttName, cmnd, message):
...
#            Debug('tasmota::updateResultDevice: sensorName: {}, sensorData: {}'.format(sensorName, sensorData))
            if sensorName == 'RfReceived':
                Debug('tasmota::updateResultDevice: sensorName: {}, sensorData: {}'.format(sensorName, sensorData))
                getSensorDeviceStateEx(states,sensorName,sensorData)
                for sensor, attr, value, desc in states:
                    ...
                    if idx == None:
                        idx = createDevice(mqttName, deviceID, cmnd, sensor, command, desc)
                        if idx != None:
                            ret = True
                    if idx != None:
    ...
    else: #POWER result of Switch 
        idxs = findDevicesByID(deviceID)
        attr, value = next(iter(message.items()))
        Debug('tasmota::updateResultDevices: attr: {}, vlaue: {}, message: {}'.format(repr(message)))
        for idx in idxs:
            try:
                description = json.loads(Devices[idx].Description)
                if description['Command'] == attr:
...
#    Debug('tasmota::deviceByNameType: device: {}, attr: {}, idxs {}'.format(device, attr, idxs))
    for idx in idxs:
        try:
            description = json.loads(Devices[idx].Description)
            if description['Device'] == device:
                if description['Type'] == attr:
                    return idx
                elif (attr == 'Temperature' or attr == 'Humidity') and description['Type'] == 'Temp+Hum' :
                    return idx
        except:
            pass
    return None
    ...
    if isinstance(message, collections.Mapping):
        for sensorName, sensorData in message.items():
            ...
            for sensor, attr, value, desc in getSensorDeviceStates(sensorName,sensorData):
                ...
                idx = deviceByNameType(idxs, sensor, desc['Name']) # desc['Name']: domoticz device type: (example: 'Temp+Hum')
                if idx == None:
                    idx = createDevice(mqttName, deviceIDSens, cmnd, sensor, command, desc)
                    if idx != None:
                        ret = True
                if idx != None:
